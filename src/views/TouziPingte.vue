    ...
    <div>
        ...
        >
            ...
            <template #item.actions="{ item }">
                <v-btn v-if="checkPermission('touzi-pingte-delete')" size="small" variant="tonal" color="error" :disabled="item.is_finished == 1" @click="deleteRecord(item.id)"><v-icon>mdi-delete</v-icon> 删除</v-btn>
            </template>
        </v-data-table-server>
