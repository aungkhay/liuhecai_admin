        ...
        >
            <v-card>
                <v-card-title class="d-flex justify-space-between align-center">
                    <span>{{ selectedId.value ? '编辑' : '添加' }}开奖记录</span>
                ...
                <v-card-text class="pa-3">
                    ...
                    <div class="d-flex justify-end mt-3">
                        <v-btn color="primary" :disabled="isSaving || v$.$invalid" @click="saveRecord"><v-icon class="mr-2">mdi-content-save-outline</v-icon> {{ selectedId.value == 0 ? '保存' : '更新' }}</v-btn>
                    </div>
                </v-card-text>
            </v-card>
        </v-dialog>
...
const formatedYears = computed(() => {
    ...
    for (const key in years.value) {
        if (Number(key) > currentYear) continue;
...
const getRecords = async () => {
    const res = await LOTTERY_RECORDS(page.value,perPage.value,obj.value.lottery_type);
    if (res.code == 1000) {
...
const deleteRecord = async () => {
    ...
    const res = await DELETE_LOTTERY_RECORD(selectedRecord.value.id, 'aomen');
    if (res.code == 1000) {
