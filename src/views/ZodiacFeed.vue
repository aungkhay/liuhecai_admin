    ...
    <div>
        ...
        >
            ...
            <template #item.actions="{ item }">
                <v-btn v-if="checkPermission('zodiac-feed-update')" size="small" variant="tonal" color="success" class="mr-2" @click="editRecord(item)"><v-icon>mdi-pencil</v-icon> 编辑</v-btn>
                <v-btn v-if="checkPermission('zodiac-feed-delete')" size="small" variant="tonal" color="error" @click="confirmDelete(item)"><v-icon>mdi-delete</v-icon> 删除</v-btn>
            </template>
        </v-data-table-server>
