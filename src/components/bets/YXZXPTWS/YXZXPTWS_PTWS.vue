    ...
    <v-card :elevation="0" max-width="900">
        <v-row dense>
            <v-col cols="12" sm="6" md="4" lg="3" v-for="(item, index) in items" :key="index">
                <div class="bet-item d-flex align-center justify-space-between pa-2 rounded cursor-pointer border" @click="toggleItem(item)" :class="{ 'active border-primary': selectedItems.includes(item) }">
                    <div>{{ item.name }}</div>
